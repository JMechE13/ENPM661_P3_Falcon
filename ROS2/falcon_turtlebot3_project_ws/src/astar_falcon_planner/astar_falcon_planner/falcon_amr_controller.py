...
class RobotAStarPlannerNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'\nstart: {int_start}, \nend: {int_end}, \nrobot_radius: {robot_radius}, \nclearance: {clearance}, \ngoal_threshold: {goal_threshold}, \nwheel radius: {wheel_radius}, \nwheel base: {wheel_distance}, \nrpms: {rpms}')

        self.get_logger().info('Beginning Path Planning...')
        ...
        if path != -1 and path != -2:
          self.get_logger().info(f'Path 1 Planned -- Points Generated: {len(path)}')
        elif path == -1: 
            self.get_logger().fatal(f'Path Planner Failed Due to Invalid Start or End Locations')
        elif path == -2:
            self.get_logger().fatal(f'Path Planner Failed -- Could Not Find Path')
        ...
        self.state = 0
        self.get_logger().info(f'Actual stop calculated: {actual_stop_pos}')
        self.iteration = 0
        self.pop_count = 0
        self.total_path_steps = len(path)
        ...
        self.abs_pos = actual_stop_pos

    def on_timer(self):
        self.get_logger().info(f'EXECUTING STEP: {self.iteration}')
        self.iteration += 1
    ...
    def publish_twist_cmd(self, planned_path):
        self.get_logger().info(f'\nProgress: ({self.pop_count} / {self.total_path_steps})')
        self.get_logger().info(f'\nx: {float(planned_path[0][0]/self.delta_time)}\ny: {float(planned_path[0][1]/self.delta_time)}\ntheta: {float(planned_path[0][2]/self.delta_time)}')
